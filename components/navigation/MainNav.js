...
const MainNav = () => {
  const items = useSelector((state) => state.navItems.navItems[0]);

  return (
    <div className="w-full h-h-40 bg-green">
      <ul className="flex w-full h-h-full justify-evenly items-center">
        <Link href='/'><li className='text-blue-dark uppercase  text-sm font-semibold hover:text-white hover:underline'>Home</li></Link>
        {items &&
          items.items.map((i) => (
